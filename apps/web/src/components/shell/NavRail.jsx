...
const workspaces = [
  { id: 'dashboard', label: 'Dashboard', icon: 'dashboard', path: '/dashboard', built: true },
  { id: 'patients', label: 'Patients', icon: 'group', path: '/patients', built: true },
  ...
  { id: 'admin', label: 'Admin', icon: 'settings', path: '/admin', built: true },
      ...
      {filteredWorkspaces.map((ws) => {
        ...
        return (
          <button
            ...
            title={isBuilt ? ws.label : `${ws.label} — Coming soon`}
            className={`
              relative w-12 h-12 flex flex-col items-center justify-center rounded-md transition-colors
              ${isActive
                ? 'bg-[#2E3A5E] text-white'
                : isBuilt
                  ? 'text-white/60 hover:bg-[#2E3A5E] hover:text-white/90'
                  : 'text-white/30 hover:bg-[#2E3A5E] hover:text-white/50'}
            `}
          >
            {isActive && (
            ...
            {!isBuilt && <span className="text-[7px] leading-none mt-0.5 opacity-60">Soon</span>}
          </button>
        );
      })}
    </nav>
