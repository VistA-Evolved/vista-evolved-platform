...
| `control-plane.tenants.identity` | `none` | `base-visible` | — | High | Identity and invitation management is governance. No pack gates it. |
| `control-plane.markets.payer-readiness` | `payer-specific` | `base-visible` | — | High | Content variation: which payers appear varies by market and payer packs. The surface itself is always visible — operators must see payer readiness across all markets. |
| `control-plane.packs.eligibility-simulator` | `multi-dimensional` | `base-visible` | — | High | Content variation: simulation evaluates pack eligibility across all dimensions (regulatory, specialty, language, payer). The surface itself is always visible — it is a governance tool for reasoning about pack outcomes. |
| `control-plane.fleet.environments` | `none` | `base-visible` | — | High | Feature flag and environment governance is infrastructure operations. Completely pack-independent. |
| `control-plane.fleet.backup-dr` | `none` | `base-visible` | — | High | Backup, restore, and disaster recovery is infrastructure governance. Completely pack-independent. |
| `control-plane.commerce.billing` | `none` | `base-visible` | — | High | Billing and entitlements is commercial operations governance. Pack-independent — billing applies to all tenants regardless of active packs. |
| `control-plane.commerce.usage` | `none` | `base-visible` | — | High | Usage metering is commercial operations governance. Pack-independent. |
| `control-plane.platform.operations-center` | `none` | `base-visible` | — | High | Cross-cutting platform health dashboard. Pack-independent — aggregates signals from all service domains. |
| `control-plane.platform.templates` | `multi-dimensional` | `base-visible` | — | High | Content variation: templates span regulatory, specialty, and language dimensions. The surface itself is always visible — operators must manage templates for all markets and packs. |
| `control-plane.platform.support` | `none` | `base-visible` | — | High | Support console is operations governance. Pack-independent. |
| `control-plane.platform.audit` | `none` | `base-visible` | — | High | Audit trail is compliance governance. Pack-independent — you cannot audit what you cannot see. |
| `control-plane.platform.alerts` | `none` | `base-visible` | — | High | Alert management is operations governance. Pack-independent. |
| `control-plane.platform.runbooks` | `none` | `base-visible` | — | High | Internal documentation hub. Pack-independent. |

**Workspace A summary:** All 21 control-plane surfaces are `base-visible`. No pack family gates control-plane surface visibility. Three surfaces have `country-regulatory` sensitivity, three have `multi-dimensional` sensitivity, one has `payer-specific` sensitivity, and fourteen have `none` — in all cases, sensitivity affects *content* (which markets are shown, which packs are displayed, which payer data appears, which templates are available) not *surface visibility*. The surfaces themselves are always visible to platform operators.

**Rationale:** Control-plane surfaces are platform-operator governance tools. Making them conditionally visible based on pack state would undermine the operator's ability to manage packs in the first place — the operator needs to see the pack catalog to activate packs, must see market management to manage markets regardless of regulatory pack state, and must see payer readiness across all markets regardless of which payer packs exist.
