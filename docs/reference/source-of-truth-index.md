...
| **Data source policy (fixture purge complete)** | `docs/explanation/fixture-inventory-and-truth-ownership-audit.md` | **UPDATED:** All fixtures deleted 2026-03-21. VistA is the sole data source for tenant-admin. No JSON fallbacks exist. |
| **Tenant admin happy-path source map** | `docs/explanation/tenant-admin-happy-path-source-map.md` | Per-surface source-of-truth map: all screens now read VistA live exclusively, no fallback. Badge is `vista` or `error`. |
...
| **Web UI (React + Vite + Tailwind)** | `apps/web/` | **Current active frontend** on port 3000. Admin workspace (10 pages) + Patient workspace (12 pages). Proxies `/api/ta/v1/*` to tenant-admin (4520) and `/api/op/v1/*` to control-plane-api (4510). Design direction from 62 product spec docs in `docs/specs/`. Replaces admin-ui, control-plane SPA, and tenant-admin SPA. |
| ~~Unified Admin UI (Next.js)~~ | ~~`apps/admin-ui/`~~ | **ARCHIVED 2026-04-04** to `_archived/apps/admin-ui/`. Too much VistA terminology, ~70 confusing routes. Replaced by `apps/web/`. See `_archived/README.md`. |
